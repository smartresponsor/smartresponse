...
#[AsController]
final class RetailPlacementApiController extends AbstractController
{
    public function __construct(
        private readonly Security $security,
        private readonly VendorRepositoryInterface $vendorRepository,
        private readonly RetailPlacementApiService $placement,
    ) {
    }
    ...
    {
        return $this->execute($retailId, static fn (RetailPlacementApiService $service, string $vendorId, string $ownerUuid): array => $service->snapshot($retailId, $vendorId));
    }
    ...
    {
        return $this->execute($retailId, fn (RetailPlacementApiService $service, string $vendorId, string $ownerUuid): array => $service->fulfillment($retailId, $vendorId, $this->payload($request)));
    }
    ...
    {
        return $this->execute($retailId, fn (RetailPlacementApiService $service, string $vendorId, string $ownerUuid): array => $service->location($retailId, $vendorId, $ownerUuid, $this->payload($request)));
    }
    ...
    {
        return $this->execute($retailId, fn (RetailPlacementApiService $service, string $vendorId, string $ownerUuid): array => $service->pricing($retailId, $vendorId, $this->payload($request)));
    }
    ...
    {
        return $this->execute($retailId, static fn (RetailPlacementApiService $service, string $vendorId, string $ownerUuid): array => $service->publish($retailId, $vendorId));
    }

    /** @param callable(RetailPlacementApiService,string,string):array<string,mixed> $operation */
