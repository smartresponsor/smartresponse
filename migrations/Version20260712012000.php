...
final class Version20260712012000 extends AbstractMigration
{
    ...
    {
        $this->addSql("CREATE TABLE IF NOT EXISTS cart_cart (id SERIAL PRIMARY KEY, cart_token VARCHAR(96) NOT NULL, owner_reference VARCHAR(191) DEFAULT NULL, currency_code VARCHAR(3) NOT NULL, status VARCHAR(32) NOT NULL, created_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updated_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, expires_at TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL, version INT NOT NULL DEFAULT 1)");
        ...
        $this->addSql("CREATE TABLE IF NOT EXISTS cart_adjustment (id SERIAL PRIMARY KEY, cart_id INT NOT NULL, type VARCHAR(32) NOT NULL, label VARCHAR(191) NOT NULL, amount_minor INT NOT NULL, CONSTRAINT fk_cart_adjustment_cart FOREIGN KEY (cart_id) REFERENCES cart_cart (id) ON DELETE CASCADE)");
